...
func (blog *Blog) JSONAPILinks() *jsonapi.Links {
	return &jsonapi.Links{"self": fmt.Sprintf("https://example.com/blogs/%d", blog.ID)}
}
...
func (blog *Blog) JSONAPIRelationshipLinks(relation string) *jsonapi.Links {
	if relation == "posts" {
		return &jsonapi.Links{"related": fmt.Sprintf("https://example.com/blogs/%d/posts", blog.ID)}
	}
